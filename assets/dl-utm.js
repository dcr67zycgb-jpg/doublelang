...
(function () {
  ...
  /* ⚠️ Имя поля ОБЯЗАНО совпадать с тем, что принимает сервер (_MARK_LIMITS). Гейта
   * между репозиториями нет и быть не может, поэтому имя вынесено сюда: при правке
   * видно, что менять его в одиночку бессмысленно. `_fbc` против `fbc` мы уже
   * проходили — поле молча не доезжает, а обе стороны выглядят исправными. */
  var CLICK_TS = 'fbclid_ts';  // когда был клик; ISO-8601 UTC
  ...
  /* Кука по имени. Нужна ровно для двух идентификаторов Meta — _fbp и _fbc, — которые
   * ставит сам пиксель на этом же домене, поэтому document.cookie их видит.
   * ⚠️ Имя ищется как ЦЕЛЫЙ ключ, а не подстрокой: «_fbc» встречается внутри чужих
   * имён, и подстрочный поиск однажды вернул бы чужое значение под нашим полем. */
  function cookie(name) {
    ...
    } catch (e) { /* куки недоступны — просто нет меток */ }
    return '';
  }
  ...
  var fresh = fromUrl();
  if (fresh) {
    /* ⚠️ ВРЕМЯ КЛИКА ЗАПИСЫВАЕТСЯ ВМЕСТЕ С МЕТКАМИ. Без него сервер, достраивая `fbc`
     * из сохранённого `fbclid`, подставлял момент ПРИЁМА заявки — и тем самым заявлял
     * Meta, что клик был сейчас. У памяти срока нет: человек мог кликнуть месяц назад,
     * прийти сегодня напрямую, и мы отдавали неверную дату, а не устаревшую.
     * Ставится в момент ВХОДА по рекламной ссылке, то есть это и есть время клика. */
    fresh[CLICK_TS] = new Date().toISOString();
    var s = '';
  ...
  function marks() {
    ...
    try {
      ...
      /* ── Идентификаторы Meta: из КУК, а не из адреса ────────────────────────────
       * `_fbp` и `_fbc` ставит пиксель Facebook на этом домене. Без них серверная
       * отправка событий (CAPI) не сматчится с кликом, и вся реклама считается
       * «без источника» — поля на сервере есть с 17.09, а значений в них не было,
       * потому что читать куки было некому.
       *
       * ⚠️ ИМЕНА ПОЛЕЙ БЕЗ ПОДЧЁРКИВАНИЯ: сервер берёт ключи из _MARK_LIMITS и знает
       * `fbc`/`fbp`. Отправь мы `_fbc`, как называется кука, — поле молча не доехало
       * бы, а выглядело бы всё исправным с обеих сторон.
       *
       * ⚠️ Достройки `fbc` из `fbclid` здесь НЕТ И БЫТЬ НЕ ДОЛЖНО. Она уже сделана на
       * сервере (`if not marks.get("fbc") and marks.get("fbclid")`), и вторая
       * реализация дала бы двух писателей одного значения с разными метками времени
       * внутри. Нет куки — шлём пусто, сервер достроит сам, если был клик. */
