...
(function () {
  ...
  function pushLead(body) {
    try {
      ...
      /* Пиксель Meta. Раньше событие Lead стояло ТОЛЬКО на thank-you.html, то
         есть считалось каждое открытие страницы благодарности — включая
         перезагрузки и возвраты по кнопке «назад». Кампании оптимизировались
         по этому сигналу, а значит алгоритм учился приводить тех, кто доходит
         до страницы, а не тех, кто оставляет заявку. Теперь событие идёт
         отсюда — из точки, где заявка реально принята сервером. */
